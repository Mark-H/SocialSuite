...
if (!$socialsuite) return '[FacebookFans] Error instantiating SocialSuite class.';

if (empty($scriptProperties['user'])) return '[FacebookFans] Error: no user defined.';

$data = array();
$cached = false;
$cacheKey = 'facebook/users/' . $scriptProperties['user'];
...
if (!$data || empty($data)) return '[FacebookFans] Sorry, something went wrong requesting the data.';
